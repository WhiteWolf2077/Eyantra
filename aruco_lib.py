...
def detect_Aruco(img):  #returns the detected aruco list dictionary with id: corners
    ...
    aruco_dict1 = aruco.Dictionary_get(aruco.DICT_5X5_1000)
    aruco_dict2 = aruco.Dictionary_get(aruco.DICT_7X7_1000)

     #+aruco.getPredefinedDictionary(aruco.DICT_5X5_1000)#aruco.Dictionary_get(aruco.DICT_4X4_50)   #creating aruco_dict with 5x5 bits with max 250 ids..so ids ranges from 0-249
    print(aruco_dict)
    print(aruco_dict1)
    print(aruco_dict2)
    ...
    if len(corners) == 0:
         corners, ids, _ = aruco.detectMarkers(gray, aruco_dict1, parameters = parameters)
    if len(corners) == 0:
         corners, ids, _ = aruco.detectMarkers(gray, aruco_dict2, parameters = parameters)
